...
static int raw_crypt(nvaes_ctx ctx, unsigned char mode, unsigned char *src, int sz, unsigned char *dest, unsigned char *iv)
{
    ...
    if (mode) {
        assert(!mbedtls_aes_setkey_enc(&priv->mb_ctx, priv->key, 128));
    } else {
        assert(!mbedtls_aes_setkey_dec(&priv->mb_ctx, priv->key, 128));
    }

    assert(!mbedtls_aes_crypt_cbc(&priv->mb_ctx,
    ...
    #ifdef NVAES_DEBUG_RAW_CRYPT
    #ifdef NVAES_DEBUG_DATA
    dump("result src", src, sz);
    dump("result dest", dest, sz);
    #endif
    #endif
